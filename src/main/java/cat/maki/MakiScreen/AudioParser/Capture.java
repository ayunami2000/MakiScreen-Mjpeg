...
package cat.maki.MakiScreen.AudioParser;

import cat.maki.MakiScreen.AudioCapture;
import cat.maki.MakiScreen.AudioParser.Analysis.Analysis;
import cat.maki.MakiScreen.MakiScreen;
import cat.maki.MakiScreen.PitchDetection;
...
public class Capture implements Runnable{
	...
	public void run() {
		...
		if(MakiScreen.audioDevice.equals(""))return;
			try{
				Mixer.Info[] mixerInfos = AudioSystem.getMixerInfo();
				Mixer mixer = null;
				for (int i = 0; i < mixerInfos.length; i++) {
					if (mixerInfos[i].getName().trim().equals(MakiScreen.audioDevice)) {
				...
				if(mixer==null){
					for (int i = 0; i < mixerInfos.length; i++) {
						try {
							int index = Integer.parseInt(MakiScreen.audioDevice);
							if (index==i) {
								mixer = AudioSystem.getMixer(mixerInfos[i]);
								break;
							}
